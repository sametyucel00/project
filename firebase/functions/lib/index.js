...
async function assertRateLimit(uid, action, maxCount, windowSeconds) {
...
export const listAdminUsers = onCall(async (request) => {
    ...
    let userQuery = db.collection("users").orderBy("createdAt", "desc").limit(100);
    const allowedRoles = ["individual", "business", "theater", "admin"];
...
export const estimateNotificationAudience = onCall(async (request) => {
    ...
    if (target.kind === "manual") {
        ...
        const tokenLists = await Promise.all(userIds.map((uid) => db.collection(`users/${uid}/fcmTokens`).limit(20).get()));
    ...
    if (target.kind === "event" && target.eventId) {
        const favorites = await db.collectionGroup("favorites")
            ...
            .limit(500)
            .get();
        const userIds = [...new Set(favorites.docs.map((doc) => doc.ref.parent.parent?.id).filter(Boolean))];
    ...
    if (target.kind === "role" && target.role)
        userQuery = userQuery.where("role", "==", target.role);
    if (target.kind === "city" && target.city)
        userQuery = userQuery.where("city", "==", target.city);
...
export const getAdminDashboardSummary = onCall(async (request) => {
    ...
    const languageUse = users.docs.reduce((totals, doc) => {
...
export const reviewApproval = onCall(async (request) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!approval.exists)
            throw new HttpsError("not-found", "Onay kaydı bulunamadı.");
...
export const completeUserTask = onCall(async (request) => {
    ...
    if (!taskId)
        throw new HttpsError("invalid-argument", "Görev kimliği gerekli.");
    const uid = request.auth.uid;
    const completionRef = db.doc(`users/${uid}/taskCompletions/${taskId}`);
    await db.runTransaction(async (tx) => {
        const completion = await tx.get(completionRef);
        if (completion.exists)
            throw new HttpsError("already-exists", "Görev zaten tamamlandı.");
...
async function recordAnalyticsEvent(input) {
    const { type, entityType = null, entityId = null, userId = null, city = "Antalya", locale = "tr", platform = "unknown" } = input;
...
export const trackAnalyticsEvent = onCall(async (request) => {
    const { type, entityType = null, entityId = null, city = "Antalya", locale = "tr", platform = "unknown" } = request.data ?? {};
    if (!type)
        throw new HttpsError("invalid-argument", "Analitik event türü gerekli.");
...
export const saveGooglePlaceSnapshot = onCall(async (request) => {
    ...
    if (!allowed)
        throw new HttpsError("permission-denied", "Bu mekan için Google Places güncellemesi yapılamaz.");
    await placeRef.set({
...
export const updateOfferStatus = onCall(async (request) => {
    ...
    if (!offer.exists)
        throw new HttpsError("not-found", "Fırsat bulunamadı.");
    const isAdminUser = user.data()?.role === "admin";
...
export const updateOrderStatus = onCall(async (request) => {
    ...
    if (!orderId || !status)
        throw new HttpsError("invalid-argument", "Sipariş kimliği ve durum gerekli.");
    const uid = request.auth.uid;
    const user = await db.doc(`users/${uid}`).get();
    ...
    if (!allowed)
        throw new HttpsError("permission-denied", "Sipariş güncelleme yetkisi yok.");
    await orderRef.set({
        status,
        updatedAt: FieldValue.serverTimestamp(),
        updatedBy: uid,
...
async function collectTargetTokens(target) {
    let userQuery = db.collection("users");
    if (target.kind === "role" && target.role)
        userQuery = userQuery.where("role", "==", target.role);
    if (target.kind === "city" && target.city)
    ...
    if (target.kind === "event" && "eventId" in target && target.eventId) {
        const favoriteUsers = await db.collectionGroup("favorites")
            ...
            .limit(500)
            .get();
        const userIds = favoriteUsers.docs.map((doc) => doc.ref.parent.parent?.id).filter(Boolean);
        const tokenLists = await Promise.all(userIds.map((uid) => db.collection(`users/${uid}/fcmTokens`).limit(10).get()));
        return tokenLists.flatMap((list) => list.docs.map((doc) => doc.data().token).filter(Boolean));
    }
    if (target.kind === "favorites") {
        const favoriteUsers = await db.collectionGroup("favorites").limit(500).get();
        const userIds = [...new Set(favoriteUsers.docs.map((doc) => doc.ref.parent.parent?.id).filter(Boolean))];
        const tokenLists = await Promise.all(userIds.map((uid) => db.collection(`users/${uid}/fcmTokens`).limit(10).get()));
        return tokenLists.flatMap((list) => list.docs.map((doc) => doc.data().token).filter(Boolean));
    }
    const users = await userQuery.limit(500).get();
...
function readNestedImportValue(row, path) {
...
function writeNestedImportValue(row, path, value) {
...
export const previewXlsxImport = onCall(async (request) => {
    ...
    if (!sheet)
        throw new HttpsError("invalid-argument", "XLSX içinde okunabilir sayfa bulunamadı.");
...
export const commitImport = onCall(async (request) => {
    ...
    if (!collection)
        throw new HttpsError("invalid-argument", "Desteklenmeyen import türü.");
...
export const createExportManifest = onCall(async (request) => {
    ...
    if (!kind)
        throw new HttpsError("invalid-argument", "Export türü gerekli.");
...
function assertLocalizedText(value, fieldName) {
    const text = value;
